...
//OBS : Se i não é usado insere o char c nessa posição.
void heap_insert(Heap h, char c, iterator i, int lr){
	if(valid(h,i)){
		if(use(h,i)){
			if(h->size == h->capacity){
				heap_resize(h, h->capacity*4);
			}

			int child = lr == 1 ? right_child(h,i) : left_child(h,i);
			if(!use(h,child)) h->size++;
			h->array[child] = c;
		}
		else{
			h->array[i] = c;
			...
			int i_height = floor(log2(i+1));
			h->height = h->height < i_height ? i_height : h->height;
		}
	}
}
...
void aux_subtree(Heap h, Heap sub, int* count, iterator h_actual, iterator sub_actual){
	if((*count) == heap_size(sub)) return ;
		...
		h->array[h_actual] = sub->array[h_actual];
		(*count)++;

		//Atualiza a altura do heap
		int actual_height = floor(log2(h_actual+1));
		h->height = h->height < actual_height ? actual_height : h->height;

		aux_subtree(h,sub,count,left_child(h,h_actual),left_child(sub,sub_actual));
		aux_subtree(h,sub,count,right_child(h,h_actual),right_child(sub,sub_actual));
	}
}
...
void heap_insert_subtree(Heap h, Heap sub, iterator i, iterator sub_root){
	...
	int i_height = floor(log2(i + 1));
	int max_heght = log2(h->capacity - 1);

	if(i_height + sub->height > max_heght){
